...
import com.zecola.project.bizmq.BiMessageProducer;
...
import com.zecola.project.constant.ChartConstant;
...
//                "{明确的数据分析结论、越详细越好，不要生成多余的注释}";
    ...
    public BaseResponse<BiResponse> getChartByAisync(@RequestPart("file") MultipartFile multipartFile,
        ...
        // chart.setGenChart(genChart);
        // chart.setGenResult(genResult);
        chart.setStatus(ChartConstant.CHART_WAIT);
        chart.setUserId(loginuser.getId());
        boolean saveResult = chartService.save(chart);
        if (!saveResult) {
        ...
        CompletableFuture.runAsync(() -> {
            ...
            updateChart.setStatus(ChartConstant.CHART_RUNNING);
            ...
            updateChartResult.setStatus(ChartConstant.CHART_SUCCEED);
...
//                "{明确的数据分析结论、越详细越好，不要生成多余的注释}";
    ...
    /**
     * 处理图表更新错误
     * 当图表更新操作由于某种原因失败时，此方法将被调用，用于更新图表的状态为“failed”，并记录错误信息
     *
     * @param chartId     图表的唯一标识符
     * @param execMessage 执行过程中遇到的错误信息
     */
    private void handleChartUpdateError(long chartId, String execMessage) {
        // 创建一个新的Chart对象，用于更新图表的状态
        Chart updateChartResult = new Chart();
        updateChartResult.setId(chartId);
        updateChartResult.setStatus(ChartConstant.CHART_FAILED);
